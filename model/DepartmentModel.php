...
function updateDepartmentById(
    ...
    $checkUpdate = false;
    $db = connectionDb();
    $sql = "UPDATE `departments` SET `name` = :nameDepartment, `slug` = :slug, `leader` = :leader, 
    `date_beginning` = :beginDate, `status` = :statusDepartment, `logo` = :logo, `updated_at` = :updated_at 
...
function getAllDataDepartmentsByPage($keyword = null, $start = 0, $limit = 2){
    $key = "%{$keyword}%";
    $sql = "SELECT * FROM `departments` WHERE (`name` LIKE :nameDepartment OR `leader` LIKE :leader ) 
    AND `deleted_at` IS NULL  LIMIT :startData, :limitData";
    $db = connectionDb();
    $stmt = $db->prepare($sql);
    $data = [];
    if($stmt){
